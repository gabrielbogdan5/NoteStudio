...
const CACHE_NAME    = 'notestudio-v9';
const RUNTIME_CACHE = 'notestudio-runtime-v9';
...
const PRECACHE_URLS = [
  ...
  '/NoteStudio/offline.html',
  '/NoteStudio/404.html',
  '/NoteStudio/about.html',
  '/NoteStudio/contact.html',
  '/NoteStudio/help.html',
  '/NoteStudio/update.html',
  '/NoteStudio/manifest.json',
  '/NoteStudio/icon-192x192.png',
  '/NoteStudio/icon-512x512.png',
  '/NoteStudio/icon-152x152.png'
];
...
self.addEventListener('fetch', event => {
  ...
  /* 4. App shell (same origin) → Cache-First with offline fallback */
...
self.addEventListener('periodicsync', event => {
  if (event.tag === 'notestudio-periodic-sync') {
    event.waitUntil(periodicSyncNotes());
  }
});

async function periodicSyncNotes() {
  try {
    const cache = await caches.open(CACHE_NAME);
    await cache.addAll(PRECACHE_URLS);
  } catch (e) {
    /* Silently fail if offline during periodic sync */
  }
}
...
self.addEventListener('push', event => {
  ...
  const title = data.title || 'NoteStudio';
  const options = {
    body: data.body || 'You have a new notification.',
    icon: '/NoteStudio/icon-192x192.png',
    badge: '/NoteStudio/icon-192x192.png',
    data: { url: data.url || '/NoteStudio/' }
  };
...
self.addEventListener('notificationclick', event => {
  ...
  const targetUrl = event.notification.data?.url || '/NoteStudio/';
  event.waitUntil(
...
/** Cache-First with explicit offline.html fallback for navigation requests */
async function cacheFirstWithOfflineFallback(request) {
  ...
  try {
    ...
    /* Offline — return offline page for navigation, app shell for the rest */
    if (request.mode === 'navigate') {
      const offlinePage = await caches.match(OFFLINE_URL);
      if (offlinePage) return offlinePage;
    }
    return (
      await caches.match('/NoteStudio/') ||
      await caches.match('/NoteStudio/index.html')
    );
  }
}
...
/** Network-First: try network; on failure fall back to cache or offline page */
